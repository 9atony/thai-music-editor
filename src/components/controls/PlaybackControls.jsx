...
const PlaybackControls = () => {
  const {
    ...
    // ⭐ ดึง State และฟังก์ชันคุมท่อนเพลงมาใช้ในฝั่ง Desktop
        ...
        return (
          <div className="flex items-center gap-4 flex-wrap animate-fadeIn">
            ...
            <div className="flex items-center gap-2">
              ...
              <select onChange={(e) => formatText('fontName', e.target.value)} className="border border-slate-300 rounded-md px-2 py-1 text-sm text-slate-700 focus:outline-none focus:border-sky-400 cursor-pointer w-28 truncate">
            ...
            <div className="flex items-center bg-slate-50 border border-slate-300 rounded-md overflow-hidden h-[30px]">
              ...
              <input type="number" min="10" max="150" value={textFontSize} onChange={handleFontSizeChange} className="w-12 text-center text-sm font-bold text-slate-700 bg-transparent border-none focus:ring-0 p-0" />
              <button onMouseDown={(e) => { e.preventDefault(); handleFontSizeStep(2); }} className="w-7 h-full text-slate-500 hover:bg-slate-200 font-black">+</button>
            </div>
        ...
        return (
          <div className="flex items-center gap-6 animate-fadeIn w-full">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <select value={layoutConfig.noteFontFamily || defaultFontFamily} onChange={(e) => handleLayoutChange('noteFontFamily', e.target.value)} className="border border-slate-300 rounded-md px-2 py-1 text-sm text-slate-700 focus:outline-none focus:border-sky-400 cursor-pointer w-24 truncate">
                  {FONT_OPTIONS.map(font => (<option key={`note-${font.value}`} value={font.value}>{font.label}</option>))}
                </select>
              </div>
              <div className="flex items-center bg-slate-50 border border-slate-300 rounded-md overflow-hidden h-[30px]">
                <button onMouseDown={(e) => { e.preventDefault(); handleFontSizeStep(-2); }} className="w-7 h-full text-slate-500 hover:bg-slate-200 font-black">−</button>
                <input type="number" min="10" max="150" value={layoutConfig.fontSize || 30} onChange={handleFontSizeChange} className="w-10 text-center text-sm font-bold text-slate-700 bg-transparent border-none focus:ring-0 p-0" title="ขนาดตัวโน้ต" />
                <button onMouseDown={(e) => { e.preventDefault(); handleFontSizeStep(2); }} className="w-7 h-full text-slate-500 hover:bg-slate-200 font-black">+</button>
              </div>
              ...
              <div className="flex items-center gap-2 ml-1 border-l border-slate-200 pl-3">
                <select value={layoutConfig.activeSymbol || 'sabat'} onChange={(e) => handlePropChange('activeSymbol', 'type', e.target.value)} className="border border-slate-300 rounded-md px-2 py-1 text-sm text-slate-700 focus:outline-none focus:border-sky-400 cursor-pointer w-24">
