...
const DesktopLayout = ({ children, currentPage, onPageChange }) => {
  ...
  return (
    ...
    >
      ...
      <aside className="w-[260px] flex flex-col bg-white border-r border-slate-100 h-screen shrink-0 relative z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        
        {/* โลโก้ด้านบน */}
        <div className="pt-10 pb-8 px-8 flex justify-center border-b border-slate-50/50">
          <img src={logoImg} alt="TME Logo" className="w-[200px] object-contain drop-shadow-sm" />
        </div>
        ...
        <div className="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl relative group transition-all ${
              currentPage === 'home' ? 'bg-white shadow-sm border border-slate-100 text-slate-900' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
            }`}
          >
            {currentPage === 'home' && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-6 bg-[#3B82F6] rounded-r-md"></div>}
            <svg className={`w-5 h-5 transition-colors ${currentPage === 'home' ? 'text-[#3B82F6]' : 'group-hover:text-slate-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            <span className={`text-[15px] ${currentPage === 'home' ? 'font-bold' : 'font-medium'}`}>หน้าหลัก</span>
          </button>

          <button 
            onClick={() => onPageChange('my-projects')}
            className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl relative group transition-all ${
              currentPage === 'my-projects' ? 'bg-white shadow-sm border border-slate-100 text-slate-900' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
            }`}
          >
            {currentPage === 'my-projects' && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-6 bg-[#3B82F6] rounded-r-md"></div>}
            <svg className={`w-5 h-5 transition-colors ${currentPage === 'my-projects' ? 'text-[#3B82F6]' : 'group-hover:text-slate-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-xs font-semibold text-[#3B82F6] mt-0.5">Premium</p>
              </div>
            </div>
