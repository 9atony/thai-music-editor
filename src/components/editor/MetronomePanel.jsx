...
const MetronomePanel = ({ isExpanded }) => {
  ...
  return <section className="border-t border-slate-200 bg-gradient-to-b from-slate-50 to-white px-2 py-2 sm:px-4" aria-label="เครื่องประกอบจังหวะ">
    <div className="w-full overflow-x-auto pb-1 custom-scrollbar">
      <div className="flex min-w-[1580px] items-center rounded-xl border border-slate-200 bg-white px-2 py-1.5 shadow-sm">
        ...
        <div className="flex w-[130px] shrink-0 items-center gap-1.5 border-l border-slate-100 px-2.5"><span className="text-[9px] font-black text-slate-500">ชั้นเพลง</span><select value={metronomeConfig.rhythmLayer || 'all'} onChange={(event) => updateRhythmLayer(event.target.value)} className="min-w-0 flex-1 rounded-md border border-slate-200 bg-slate-50 px-1.5 py-1 text-[10px] font-bold text-slate-700 outline-none" aria-label="กรองหน้าทับตามชั้นเพลง">{RHYTHM_LAYER_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>
        {cards.map(({ key, label, Icon, icon, active }) => { const c = metronomeConfig[key]; const patterns = filterRhythmPatternsByLayer(metronomeConfig.rhythms[key], metronomeConfig.rhythmLayer); const isUnavailable = patterns.length === 0; return <div key={key} className={`flex min-w-[370px] flex-1 items-center gap-2 border-l border-slate-100 px-3 ${c.active ? '' : 'opacity-60'}`}>
