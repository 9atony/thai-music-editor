...
const MyProjects = ({ onNewProject, onOpenArrangerProjects, userProfile }) => {
  ...
  useEffect(() => {
    const handlePointerDownOutside = (event) => {
      if (event.target.closest('[data-project-menu], [data-project-sort-menu]')) return;
      setOpenMenuId(null);
  ...
  const calculateTotalStorageUsed = () => storageInfo?.usedBytes || 0;
  ...
  const isCreationAllowed = (incomingFileSize = 0) => {
    ...
    if (userRole === 'user') {
      ...
      return (storageInfo?.projectCount ?? projects.length) < 10;
    } 
  ...
  const handleOpenProject = async (project) => {
    ...
    try {
      ...
      recordSystemEvent('projectOpens', { projectId: project.id });
  ...
  const confirmRename = async () => {
    ...
    try {
      const uid = auth.currentUser?.uid;
      await renameProjectInDB(uid, projectToRename.id, newProjectName.trim());
  ...
  const handleDuplicateProject = async (e, project) => {
    ...
    const duplicateSize = project.storageSizeBytes || 0;
    ...
    try {
      const duplicateId = await duplicateProjectInDB(uid, project.id, duplicateName, { summary: project, userProfile });
  ...
  const handleDeleteProject = async (e, projectId) => {
    e.stopPropagation();
    if (deletingProjectId || !window.confirm("คุณต้องการลบโปรเจกต์นี้อย่างถาวรใช่หรือไม่?")) return;
    ...
    try {
      await deleteProjectFromDB(uid, projectId);
      setProjects(prev => prev.filter(project => project.id !== projectId));
      setStorageInfo(await getUserStorageUsage(uid, userProfile));
      setOpenMenuId(null);
    } catch (error) {
      console.error('ไม่สามารถลบโปรเจกต์ได้:', error);
  ...
  const handleLoadMore = async () => {
    const uid = auth.currentUser?.uid;
    if (!uid || !pageCursor || isLoadingMore) return;
    setIsLoadingMore(true);
    try {
      const page = await fetchProjectSummaries(uid, { cursor: pageCursor });
  ...
  return (
    <div className="app-page-shell app-page-with-floating-footer animate-fadeIn text-slate-800 flex flex-col" style={{ fontFamily: 'Prompt, sans-serif' }}>
      ...
      <button type="button" onClick={onOpenArrangerProjects} className="mb-6 flex w-full items-center justify-between rounded-2xl border border-violet-200 bg-violet-50 px-5 py-4 text-left transition-colors hover:bg-violet-100">
        <span>
          <span className="block text-sm font-bold text-violet-900">โปรเจกต์จัดวงดนตรี</span>
          <span className="mt-0.5 block text-xs text-violet-700/70">จัดการแทร็ก, Timeline และ Mixer แยกจากโน้ตเพลงใน Editor</span>
        </span>
        <span className="rounded-xl bg-violet-600 px-3 py-2 text-xs font-bold text-white">เปิดรายการ</span>
      </button>
        ...
        <div className="flex md:hidden overflow-x-auto gap-4 pb-4 snap-x hide-scrollbar -mx-5 px-5">
          {recentProjects.map((project) => (
            <div key={`hscroll-${project.id}`} className="snap-start shrink-0 w-[140px] bg-white p-3 rounded-2xl border border-slate-100 shadow-sm flex flex-col relative text-left" onClick={() => handleOpenProject(project)}>
              <div data-project-menu className="absolute top-2 right-2 z-20">
      ...
      <div className="flex flex-col min-h-[250px]">
        <div className="flex items-center justify-between mb-4">
          ...
          <div data-project-sort-menu className="relative z-30">
                ...
                {displayedProjects.map((file) => (
                  <tr key={`list-${file.id}`} onClick={() => handleOpenProject(file)} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group cursor-pointer relative">
                    ...
                    <td className="py-3 px-5 text-right relative">
                      <div data-project-menu className="relative inline-block z-10">
            ...
            <div key={`mlist-${file.id}`} onClick={() => handleOpenProject(file)} className="flex items-center p-3 bg-white border border-slate-100 shadow-sm rounded-2xl active:scale-[0.98] transition-transform w-full text-left relative">
              ...
              <div className="flex-1 overflow-hidden">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span className="text-[8px] bg-slate-100 text-slate-400 px-1.5 py-0.5 rounded font-bold">{formatSize(file.storageSizeBytes)}</span>
                </div>
              </div>
              <div data-project-menu className="relative pl-2 h-full flex items-center">
      ...
      {hasMoreProjects && (
        <button
          ...
          className="mx-auto mb-6 rounded-xl border border-slate-200 bg-white px-5 py-2 text-xs font-bold text-slate-600 transition-colors hover:border-sky-300 hover:text-sky-600 disabled:opacity-50"
        >
          {isLoadingMore ? 'กำลังโหลด...' : 'โหลดโปรเจกต์เพิ่มเติม'}
        </button>
      )}

      <ProjectStatusBar
        role={userRole}
        itemCount={storageInfo?.projectCount ?? projects.length}
        usedBytes={calculateTotalStorageUsed()}
        itemLabel="ไฟล์"
      />
